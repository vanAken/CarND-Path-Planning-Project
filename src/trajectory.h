...
{   const long max_waypoints = 500;
    ...
    next_y = {previous_path_y[0],previous_path_y[1]};   // as a startpoint for the next Trajectory
    ...
    for(int i = 0; i < next_s.size(); i++) { // over  6945.554m -15 starts at 0 again
        if (1000 < next_sd0[0] || next_s[i] < 6000  || next_sd0[0]+6945 + v_t*1.5 < next_s[i] ){// close loop fix
            if (next_sd0[0]+ v_t*1.5  < next_s[i]){ // v_t*1.5 is around 30m by full speed
                if (next_s[i-1]+v_t/2 < next_s[i]) {  // check next_s is rising more than 12m
                    ...
                    next_xy = frenet.sd_to_xy( next_s[i]+v_t/10, next_d[i]); // double point to avoid overswinging
                    spline_points_x.push_back( next_xy[0] );
                    spline_points_y.push_back( next_xy[1] );
                    spline_points_v.push_back( next_v[i]  );
                }
    ...
    while( next_x.size() < max_waypoints) {
        if (v_t <= spline_v(32+dl) ) v_t = std::min(v_max, v_t + a_max/3 * dt); // +32 for future v
        ...
        double d_max = std::min( a_max, d_pp )/2*dt*dt; // d must be limmted by given a_max and calculated the maximal d(a_max)
