...
{   const long max_waypoints = 999;
    ...
    for(int i = 0; i < next_s.size(); i++) { // over  6945.554m -15 starts at 0 again
        if (next_sd0[0] + v_t*1.5 < next_s[i]) {  // 
            if (1000 < next_sd0[0] || next_s[i] < 6000  || next_sd0[0]+6945 + v_t*1.5 < next_s[i] ){ 
                ...
                spline_points_y.push_back( next_xy[1] );
                spline_points_v.push_back( next_v[i]  );
                std::cout << "  next_sd0[0]: " <<  next_sd0[0] << " i: " << i <<
                             "  next_s[i]: " <<  next_s[i] <<  
                             " spline_points_x: " << spline_points_x[spline_points_x.size()-1] << 
                             " spline_points_y: " << spline_points_y[spline_points_x.size()-1] << std::endl;
            }
        }
    }
