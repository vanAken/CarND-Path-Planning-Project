...
{  
   ...
   while (next_x.size() > 40){ // and cut to 1s = 50*dt in the future as needed for a* latency 
        next_x.pop_back();
        next_y.pop_back();
    }
    ...
    next_sd0 = frenet.xy_to_sd(spline_points_x[1],spline_points_y[1]);
        for(int i = 0; i < next_s.size(); i++) {
            if ( next_s[i] > 6900)  std::cout << i << " ###next_s[i]" << next_s[i] << std::endl; // howerver 6945,554÷4 = 1736,3885  
            if (next_sd0[0]+30 < next_s[i]){ // is next_s from A* still 30m in front of spline_points?
  ...
  // std::cout << "############################################# "<< std::endl;
    ...
    while( next_x.size() < max_waypoints) {
        if (v_t <= spline_v(30+dl) ) v_t = min(v_max, v_t + a_max/2 * dt); // +16 for future v
