...
private:
    ...
    double   discrete_to_v (double v, double pre_t);
    ...
    for (int i = 0; i < previous_path_x.size(); i++) {  
         ...
         if (previous_s < ::d_horizont_s){
             ::time_road[previous_d + 
                          ...
                          (i+50)/50   * ::num_of_lanes * ::d_horizont_s]= 0;
         }
         //cout << i << " " << i/50 << previous_s << " previous_d" << previous_d << endl;
    } 
...
void Prediction::search(double ego_s, double ego_d, double ego_v, double v_max, double a_max, vector<vector<double>> sensor_fusion, double d_dt) {
    ...
    while(SearchCount < NumSearches){
        ...
        nodeStart.v = discrete_to_v( ego_v, d_dt);// discrete velocity 
        ...
        if  ( SearchState == AStarSearch<MapSearchNode>::SEARCH_STATE_SUCCEEDED ){
            ...
            for(; ; ){ // endless
                ...
                //cout << " d_s: "<< d_s << " d_d: "<< d_d << " d_v: "<< d_v << " d_t" << d_t << endl;
            ...
            // follow mode calculate distance to the next car
            double other_vx;
            double other_vy;
            double other_v; 
            double other_s; 
...
double Prediction::discrete_to_v(double v, double d_dt) { // every discrete m/s one area                      
    return  v * d_dt/::discrete ;    
}
